...
const  Book2 = mongoose.model('Book2details',Book2Schema);
  //define mess Schema
...
//  login part
 route.post("/login", async (req, res) => {
    try {
      ...
      res.json({status: true,message: "login success",userType: user.userType,username: user.username});
    ...
    const role =req.userType;
        try {
            if(role!== "admin"){
                ...
                const allbooking = await Book2.find({});
    ...
    route.post('/renting', async (req, res) => {
      try {
          ...
          if (checkcar) {
             ...
             const existbook2 = await Book2.findOne({ carnumber: carnumber });
             console.log(existbook2, "car in the collection of Booking");
             ...
             const newbook2 = new Book2({
                 username,firstname,lastname,address,mobileno,emailid,carname,carnumber,pickup,destination,pickupdate,pickuptime,returndate,returntime,
             });
  ...
  //to fetch the car details
      ...
      route.get('/viewbookinguser', async(req,res)=>{
        try {
          ...
          // Fetch all booking from MongoDB
          const userbooking = await Book2.find({username:qusername});
